...
function createGalleryImgs(imgJSON) {
    for (let gallery in imgJSON) {
        ...
        for (let image of imgJSON[gallery]) {
            divEl.append("<div class='col-12 col-sm-6 col-md-3'><img class='w-100' src='img/artwork/"+gallery+"/"+image+".jpg' data-bs-target='#"+gallery+"-carousel' data-bs-slide-to='"+index+"'></div>");
            index += 1;
        }
    }
}
...
function createCarouselImgs(imgJSON) {
    ...
    for (let gallery in imgJSON) {
        ...
        for (let image of imgJSON[gallery]) {
            ...
            var l7 = "<img class='d-block w-100' src='img/artwork/"+gallery+"/"+image+".jpg' data-bs-target='#"+gallery+"-carousel'></div>"
            html += l6;
            html += l7;
            index += 1;
        }
